...
//            field.set(Blocks.co, glassBlast);
    ...
    /**
     * Serialize a full ItemStack to a base64 string, preserving raw NBT (Forge mod items,
     * custom model data, player heads, capabilities, etc).
     */
    @Override
    public String serializeItemStack(org.bukkit.inventory.ItemStack itemStack) {
        ...
        try {
            NBTTagCompound tag = nms.b(new NBTTagCompound());
    ...
    /**
     * Deserialize a base64 string written by {@link #serializeItemStack(org.bukkit.inventory.ItemStack)}
     * back into a Bukkit ItemStack (including Forge mod item data on hybrid servers).
     */
    @Override
    public org.bukkit.inventory.ItemStack deserializeItemStack(String base64) {
        if (base64 == null || base64.isEmpty()) return null;
        try {
            byte[] bytes = Base64.getDecoder().decode(base64);
            NBTTagCompound tag = NBTCompressedStreamTools.a(new ByteArrayInputStream(bytes));
            if (tag == null) return null;
            ItemStack nms = ItemStack.a(tag);
            if (nms == null) return null;
